...
> ⚠️ **ARCHIVE PRÉ-PIVOT (annotée le 2026-08-29).** Ce document est antérieur au pivot fondateur du 2026-08-26 : **app gratuite** (plus de paywall — tout le §7 pricing et le principe « backend financé par Élite » du §6 sont caducs) et **coach-only** (plus aucun utilisateur athlète — app athlète iPhone, jonction athlète par QR, profil carrière, comptes parents, partage joueur RLS caducs ; assistants = mêmes droits que le head coach, messagerie et staff & permissions retirés). Restent la référence : le design Mat Nuit (§2), la navigation 5 espaces (§3), Terrain/Séances 2.0 (§5), SportPack (§8 — déclencheur à reformuler en « signal de demande »), le pari vidéo. Voir `docs/Pivot_CoachFirst_Plan.md` et `docs/Journal_Pivot_CoachFirst.md`.
