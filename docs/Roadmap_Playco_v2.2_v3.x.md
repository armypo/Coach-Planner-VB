...
> ⚠️ **ARCHIVE PRÉ-PIVOT (annotée le 2026-08-29).** Roadmap antérieure au pivot fondateur du 2026-08-26 (app gratuite + coach-only). Caducs : la fiche **2.10 Paywall v3** (produits Élite/Entraîneur, quotas, grandfathering, SeuilUpgradeService, anti-steering C7), la décision fondateur #4 (pilote hors paywall), les actions humaines « ASC accords payants / produits Prêt à soumettre », le critère GO/NO-GO « infra ≤ 40 % du delta Élite−Pro » (le GO/NO-GO vidéo se rejuge à la rétention TelemetryDeck seule), **2.9.2 partage joueur** (plus d'utilisateur athlète), la messagerie (D1) et staff & permissions (D6) dans l'espace Équipe. Le plan d'exécution courant est `docs/Pivot_CoachFirst_Plan.md` (chantiers A-E) ; cette roadmap reste la référence des patchs déjà livrés (2.2.a→2.4) et du pari vidéo H2.
...
| **2.6.2** | ✅ 2026-07-07 (294/294, revue : 7 trouvailles corrigées — régénération à chaque tap via sheet [jamais de PDF périmé], nom de fichier par id, erreurs affichées+Logger, troncatures annoncées) | PDF « Plan de pratique » une page (heure·exercice·diagramme miniature·consignes + présences à cocher — `PDFExportService` rodé). Casable dans la fenêtre 2.2.5+ | 2-3 j |
| **2.3** | ◐ code livré 2026-07-07 (298/298) | **Livré** : LienInvitation (construction/analyse STRICTE du lien https://playco.app/join/{codeEquipe}/{codeInvitation} + QR CoreImage), onOpenURL (hors DEMO) → pré-remplissage de la jonction SIWA existante (LoginView — rejoindreEquipe reste seule autorité), QR sur la fiche joueur, écran « Inviter l'équipe » projetable (grille QR fullScreen). **Revue sécurité : 11 trouvailles corrigées** (révocation de code désormais opérante [fetch-puis-modifier Public DB — les 4 autres publierX à durcir pareil : SUIVI], confirmation « Rejoindre « Élans » ? » anti-phishing avant rattachement Apple ID, onOpenURL à la racine + rejeu du lien scanné avant login, grille projetable = athlètes seulement, https strict + ASCII strict, CIContext partagé). **Reste (actions humaines)** : domaine playco.app + AASA hébergé + entitlement Associated Domains (les QR scannés ouvrent Safari en attendant), CI Xcode Cloud, phase 0 SportPack déjà ✅ | 2,5 sem |
| **2.3.1** | ✅ 2026-07-06 (290/290) | Duplication « Continuer » (arrivées → départs, seuil 0.08) + **preset cadrage demi-terrain** : TypeTerrain.demiTerrain (rawValue inédit, legacy ?? .indoor), rendu dédié H/V (filet au bord, ligne 3 m au tiers), ratios 1:1 via ratioHorizontal/ratioVertical (éditeur + présentation), coordonnées 0-1 intactes. Revue adversariale : 6 trouvailles confirmées, corrigées (formations REMAPPÉES sur demi-terrain — distance au filet projetée sur la largeur, miniatures carrées filet au bord, cour vraiment carrée marges symétriques, rendu vertical mort supprimé) | 2-3 j |
| **2.3.2** | ✅ 2026-07-06 (commits `50b2164`+fix revue, 287/287) | Match éclair (2 champs, compo héritée À LA CRÉATION — validée contre l effectif actif+disponible, jamais depuis un match futur) + composition persistante (CompositionMatchView). **ÉCART : promotion MatchCalendrier→Seance RETIRÉE en revue** — modèle dormant depuis 9679ef2 (le wizard crée des Seance) ; champ `Seance.matchCalendrierID` conservé au schéma (additif), inutilisé. Revue adversariale : 12 trouvailles confirmées, toutes traitées | 1 sem |
| **2.4** (+2.4.1) | ◐ vague 1 noyau livré 2026-07-07 (303/303) | **Livré (A/B/C)** : tokens MatNuit + CONTRAT de contraste WCAG exécutable (MatNuitTests — encre AAA, 5 tons ≥4,5:1, encre3 prouvé décoratif), verre sombre 3.0 dans les corps (teinte ≤12 %, bordure 9 %, reflet — signatures intactes), nuit par défaut (.preferredColorScheme(.dark)), fond AccueilView UNI (gradients d ambiance morts), PaletteMat remappée vers les tons neutres (4 noms survivent, hex vifs morts). **Revue Mat Nuit : 10 trouvailles corrigées** (PencilKit overrideUserInterfaceStyle=.light — fidélité des dessins existants ; UN SEUL preferredColorScheme — toggle lune/soleil et modeSombre supprimés ; SIWA .whiteOutline ; GlassChip plafonné 12 % ; courtside PRÉSERVÉ via env modeBordDeTerrain dans les corps du kit ; tokens bordure/reflet/ombre sortis du dur ; sémantiques positif/negatif/attention alignées MatNuit ; Matchs en ton brique). **Écarts/reste (vague 1bis)** : échelle typo 11 tokens, purge .rounded/.hierarchical globale, 5 glyphes tab bar (dessin), empty states maison, repli Dynamic Type, unification des deux noirs (#0D0D0F vs systemBackground), aplatissement verre-sur-verre (ScoutingLecture), sweep .red→brique hors racine Matchs | 2,5-3 sem |
| **2.5a** | ⏭ GLISSÉE (nuit du 6-7 juil. close à 07h00 — règle des 2 h respectée) | Coquille navigation 5 espaces — inchangée, prochaine session dédiée. La carte héro tappable et les teintes d'espace (tons neutres, verre ≤12 %) sont PRÊTES côté kit (2.4) | 2,5-3 sem |
