...
| 2.0.0 | **Login unifié** — Suppression de RejoindreEquipeView : plus de code équipe à saisir en UI (reste en interne pour FiltreParEquipe/scoping). LoginView refondu : picker 3 tabs (Coach/Assistant/Athlète), post-auth role check qui déconnecte et affiche « Mauvais type de compte » si mismatch. Rôle .assistantCoach ajouté à RoleUtilisateur (hérite des permissions coach dans PermissionsRole.swift). Identifiant nouveau format `prenom.nom.XXXX` (4 chiffres aléatoires, retry 1000×, fallback user.XXXX ou UUID tronqué). Mot de passe athlète/assistant auto-généré format `LLLLL_DD` (5 lettres sans I/L/O + underscore + 2 chiffres sans 0/1). Nouveau modèle CredentialAthlete (private CloudKit DB uniquement, jamais publié via CloudKitSharingService) pour stocker les mdp en clair côté coach. ConfigMembresView : SecureField remplacé par affichage monospace + bouton dé pour régénérer. ConfigurationView.finaliser() crée des CredentialAthlete pour chaque athlète/assistant et présente IdentifiantsRecapSheet bloquante (interactiveDismissDisabled) avant d'ouvrir l'app. AjoutUtilisateurView : auto-gen identifiant + mdp pour .etudiant / .assistantCoach, sheet récap 1-item. Nouvelle vue IdentifiantsEquipeView (ProfilView → Organisation) : liste athlètes + assistants, copier/partager/régénérer mdp (sel+hash+iterations mis à jour en parallèle de motDePasseClair). PlaycoApp : EcranLancement `.rejoindre` → `.login`, dispatch vers LoginView avec closures onRetour/onConnecte. Paywall v2 hors scope — sera branché ensuite sur la même gate post-auth. Tests Swift Testing : UtilisateurIdentifiantTests + CredentialAthleteTests. |
| 2.1.0 | **Paywall v2.0 (Pro + Club)** — 2 tiers cumulatifs : Pro (coach + staff, 14,99 $/mois · 149,99 $/an) et Club (Pro + athlètes, 25 $/mois · 250 $/an), essai 14 jours Apple natif (Introductory Offer partagé au niveau Subscription Group `playco.pro`). Nouveau modèle @Model `Abonnement` (private CloudKit, cache du statut coach). `Equipe.tierAbonnementRaw` (publié publiquement pour la gate athlète multi-Apple-ID). `IdentifiantsIAP` (4 product IDs + tier lookup + clés UserDefaults cache). `TextesPaywall` (toutes chaînes FR centralisées : features Pro/Club, CTA conditionnel éligibilité essai, bannières, erreurs gate, mention auto-renouvellement légale). `StoreKitService` (@Observable) : chargerProduits, acheter (reject .unverified strict), restaurer, observerTransactions, statutSouscriptionActif. `AbonnementService` (@Observable) : enum Statut 10 cas (chargement/aucun/essaiActif/pro|clubMensuel|Annuel/gracePeriode/essaiExpire/expire), computed tierActif + peutEcrire + peutConnecterAthletes + joursRestantsEssai + doitAfficherBanniere, cache UserDefaults pour éviter flash paywall, `migrerAssistantsVersNouveauRole` (reclassement one-shot pré-v2.0), `propagerTierAuxEquipes`, `rafraichir` mapping complet 8 états StoreKit. UI paywall : `ComposantsPaywall` (FeatureRow, PricingCard avec badge `-17%` + `14 jours offerts`, SelecteurPeriode, BadgeStatut, BanniereEssai), `PaywallView(mode: .welcome|.bloquant|.gestion)` canonique 2 cards Pro+Club toggle mensuel/annuel CTA conditionnel, `BienvenuePaywallView` (wizard), `GestionAbonnementView` (ProfilView), `PaywallBloquantView` (fullScreenCover actions write), `BanniereAbonnementView` (top ContentView essai J-3/expiré/grace). PlaycoApp : injection `StoreKitService`/`AbonnementService`, ajout `Abonnement.self` au schema, orchestration boot (attendreSyncInitiale → restaurerSession → migrerAssistants → chargerProduits → rafraichir → observerTransactions), gate centrale `appliquerGateTier` (athlète requiert .club, assistant requiert .pro/.club, sinon déconnexion + alert). LoginView `onConnecte` callback PlaycoApp appelle la gate. ContentView : `onChange(of: abonnementService.statut)` déconnecte athlète runtime si tier drop, `BanniereAbonnementView` en safeAreaInset top. ConfigurationView : enchaîne IdentifiantsRecapSheet → BienvenuePaywallView bloquant avant `onTermine()`. ProfilView : nouvelle section « Mon abonnement » (coach payant uniquement) avec BadgeStatut + NavigationLink GestionAbonnementView. `FeatureGating.bloqueSiNonPayant(source:)` (modifier) appliqué sur MatchsView / ListeSeancesView / ExportStatsView (les 7 autres vues write à gater incrémentalement). AnalyticsService : 8 événements paywall (paywall_affiche/_ferme, essai_demarre/_expire, achat_initie/_reussi/_echoue, restauration_tentee). Entitlements : IAP géré par Xcode capability (pas d'entitlement explicite). TODO user : créer Subscription Group + 4 produits dans App Store Connect (IDs `com.origo.playco.{pro,club}.{mensuel,annuel}`, prix CAD, Free Trial 14j First-time, Family Sharing OFF), créer `Playco.storekit` dans Xcode pour tests sandbox. |
